...
/**
 * The public v2 wizard stores mobiles as national 9 digits ("5XXXXXXXX"); the edit
 * form (and the server rule ^05\d{8}$) expects "05XXXXXXXX". Canonicalize on load so an
 * unchanged number re-submitted from the dashboard is never rejected.
 */
export const MOBILE_FIELD_KEYS = new Set([
...
export function normalizeFieldValue(value, key) {
  ...
  if (MOBILE_FIELD_KEYS.has(key)) {
    if (value === null || value === undefined || value === "") return "";
    return formatSaudiMobileDisplay(value);
  }
