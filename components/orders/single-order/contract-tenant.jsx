...
function ContractTenant({ data }) {
  ...
  // `copy_of_the_authorization_or_agency` is shared with the OWNER's agent (step 3). When the
  // owner side has an agent, the backend keeps the tenant representative's document in
  // `copy_of_the_owner_record` instead, so read that first in that case.
  const ownerHasAgent = ["1", 1, true, "true"].includes(
    pickFirst(data?.contract_summary?.add_legal_agent_of_owner, data?.add_legal_agent_of_owner)
  );
  const agencyDocumentUrl = resolveAgencyDocumentUrl({
    copy_of_the_authorization_or_agency: ownerHasAgent
      ? pick(
