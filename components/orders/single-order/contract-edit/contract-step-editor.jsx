...
import { validateContractFields } from "@/src/lib/contract-field-validation";
...
export const ContractStepEditor = forwardRef(function ContractStepEditor(
  ...
  const handleSave = async () => {
    ...
    // Pattern validation (national id / mobile / commercial registry) mirroring the server.
    const patternErrors = validateContractFields(fields, form, isFieldVisible);
    if (Object.keys(patternErrors).length > 0) {
      setFieldErrors((prev) => ({ ...prev, ...patternErrors }));
      toast.error(Object.values(patternErrors)[0]);
      return;
    }
    ...
    try {
      ...
      setInitial({ ...form });
      setEditing(false);
      // Section editor dialogs (order detail cards) close once the save succeeds.
      if (formOnly && sectionEditorDialog?.onClose) sectionEditorDialog.onClose();
    } catch (err) {
