...
export function computeEmployeePerfMetrics(item, maxCompleted) {
  ...
  // An employee with no activity at all must not inherit the "no data => 100%"
  // SLA defaults (which would otherwise yield an inflated ~80/100 score for an
  // off-duty employee who received/processed/completed nothing).
  const hasActivity = received > 0 || openNow > 0 || completed > 0;
